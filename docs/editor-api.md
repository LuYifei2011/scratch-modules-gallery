...
- **字符编码**：UTF-8
- **错误响应格式**：`{ "error": "错误说明" }`
- **模块 ID 格式**：编辑器 API 仅接受符合 `^[a-z0-9-]+$` 的模块 ID（如 `my-module-01`），不支持大写字母、下划线、点或其他特殊字符。构建系统支持任意目录名（如以 `.` 开头的测试目录 `.test`），但这些模块无法通过编辑器 API 访问或管理。
...
脚本文件存储于 `content/modules/:id/scripts/` 目录，文件名格式为 `<order><sep><scriptId>.txt`，其中分隔符 `<sep>` 可为连字符 `-`、下划线 `_` 或空格（如 `01-main.txt`、`02_helper.txt`、`03 utility.txt`）。没有数字前缀的文件（如 `main.txt`）也受支持，此时 `order` 为 `0`。

> **注意：** 脚本列表按文件名**字母序**返回（如 `1-`、`10-`、`2-` 的字母序为 1→10→2），与构建系统的自然数字序（1→2→10）不同。建议统一使用零填充数字前缀（如 `01-`、`02-`）确保两者顺序一致；通过编辑器 API 创建的脚本文件始终使用两位零填充格式（如 `01-main.txt`）。
