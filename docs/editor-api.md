...
- **字符编码**：UTF-8
- **错误响应格式**：`{ "error": "错误说明" }`
- **模块 ID 格式**：编辑器 API 接受符合 `^[a-z0-9.-]+$` 的模块 ID（如 `my-module-01`、`.test`），支持小写字母、数字、连字符和点，不支持大写字母、下划线或其他特殊字符。
...
> **注意：** 脚本列表按文件名**自然数字序**返回（与构建系统一致），通过编辑器 API 创建的脚本文件始终使用两位零填充格式（如 `01-main.txt`）。
