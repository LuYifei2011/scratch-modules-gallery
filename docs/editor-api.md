...
> **注意：** 所有端点仅在开发服务器运行时可用（`bun run dev`），不对外暴露。
...
- **字符编码**：UTF-8
- **错误响应格式**：`{ "error": "错误说明" }`
- **模块 ID 格式**：编辑器 API 接受符合 `^[a-z0-9.-]+$` 的模块 ID（如 `my-module-01`、`.test`），支持小写字母、数字、连字符和点；不支持大写字母、下划线、路径分隔符或目录穿越。
...
| 参数 | 说明                                                  |
| ---- | ----------------------------------------------------- |
| `id` | 模块 ID（只允许小写字母、数字、连字符和点，路径安全） |
...
```json
{
  ...
  "hasDemo": true,
  "assets": [{ "filename": "screenshot.png", "size": 102400 }]
}
```
...
脚本文件存储于 `content/modules/:id/scripts/` 目录，文件名格式为 `<order><sep><scriptId>.txt`，其中分隔符 `<sep>` 可为连字符 `-`、下划线 `_` 或空格（如 `01-main.txt`、`02_helper.txt`、`10 清理.txt`）。没有数字前缀的文件（如 `main.txt`）也受支持，此时 `order` 为 `0`。

> **注意：** 脚本列表按文件名**自然数字序**返回（与构建系统一致），通过编辑器 API 创建或重命名的脚本文件始终使用两位零填充格式（如 `01-main.txt`）。脚本 ID 使用文件名去掉数字前缀、分隔符和 `.txt` 后缀后的部分，并会 `trim()` 首尾空白。
...
| 字段      | 类型     | 说明                                                        |
| --------- | -------- | ----------------------------------------------------------- |
| `id`      | `string` | 脚本 ID（文件名去掉数字前缀、分隔符和 `.txt` 后缀后的部分） |
| `order`   | `number` | 排序序号（文件名前的数字前缀；无前缀时为 `0`）              |
...
| 字段      | 类型     | 必填 | 说明                                                                                                |
| --------- | -------- | ---- | --------------------------------------------------------------------------------------------------- |
| `id`      | `string` | ✅   | 脚本 ID，可使用中文、空格、下划线、点等文件名安全字符；禁止空值、路径分隔符、`.`、`..`、`.txt` 后缀 |
| `content` | `string` | ❌   | 脚本内容，默认为空                                                                                  |
| `order`   | `number` | ❌   | 排序序号，省略时自动为现有最大序号 + 1；创建文件时序号会以两位数字零填充（如 `01`）                 |
...
更新脚本内容，或对脚本进行重命名/重新排序。`:scriptId` 为脚本的 `id`，即文件名去掉序号前缀、分隔符和 `.txt` 后缀后的部分（例如文件 `01-main.txt` 对应的 `scriptId` 为 `main`，`02_初始化.txt` 对应 `初始化`）。
...
仅更新内容时（`id` 与 `order` 均无变化）：

```json
{ "message": "Script updated successfully" }
```

重命名或重新排序时（`id` 或 `order` 任一变化）：

```json
{ "message": "Script updated successfully", "id": "renamed-script", "order": 3 }
```

> **注意：** 只有当 `id` 与 `order` **均未发生变化**（即仅更新了 `content`）时，响应中才不包含 `id`/`order` 字段；只要其中任一发生变化，响应中两个字段都会包含。
...
- 后端实现：[`scripts/lib/editor-api.ts`](../scripts/lib/editor-api.ts)
- 路由注册：[`scripts/dev-server.ts`](../scripts/dev-server.ts)
