...
为避免简繁中文描述出现语义漂移，`zh-cn` 与 `zh-tw` 使用同源策略：如果其中一种中文已有描述，缺失的另一种会从已有描述等义派生；如果两者都缺失且本次都要生成，会先生成 `zh-cn`，再由 `zh-cn` 派生 `zh-tw`。英文与中文仍会独立生成，以适配不同长度和表达习惯。

默认 markdown 输出会显示进度，例如 `[3/12] Generating fps [zh-cn]...` 和完成状态；`--format=json` 会保持纯 JSON 输出，不显示进度，便于重定向或脚本解析。
