...
export function parseContributors(raw) {
  ...
  if (typeof raw === 'string') {
    return raw.split(',').map(s => s.trim()).filter(Boolean).map(normalizeOne).filter(Boolean);
  }
  return [];
}
...
export function buildModuleRecord(meta, extra) {
  ...
  const record = {
    ...
    scripts: Array.isArray(extra.scripts) ? extra.scripts : (extra.script ? [{ title: '', content: extra.script }] : []),
