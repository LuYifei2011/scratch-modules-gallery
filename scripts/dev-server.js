...
// --- HTTPS 配置 ---
const HTTPS_ENABLED = String(process.env.HTTPS || "").toLowerCase() === "1" || String(process.env.HTTPS || "").toLowerCase() === "true";
...
if (HTTPS_ENABLED) {
  try {
    if (HTTPS_PFX) {
      ...
      httpsOptions = { key: fs.readFileSync(path.resolve(HTTPS_KEY)), cert: fs.readFileSync(path.resolve(HTTPS_CERT)) };
    } else {
      // 自动生成自签名证书（开发用途）
